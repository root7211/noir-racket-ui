...
| `ACKNOWLEDGED_ROW_STATE_PLAN_ABI_V1.md` | Alerts确认行状态的固定bitset ABI、应用层lowering、Rust admission/proof、幂等确认和recycle恢复规范。 |
| `ACKNOWLEDGED_ROW_STATE_PLAN_V1_REPORT.md` | 固定word表、首次确认、滚动recycle恢复、重复确认零写入和真实X11/Vulkan验证报告。 |
| `tools/mutate_acknowledged_row_state_scene.py` | 生成确认状态ABI、required、word表几何与Alerts owner四类攻击Scene。 |
