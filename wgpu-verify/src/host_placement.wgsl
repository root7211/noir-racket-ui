...
// glyph_words 的每个 legacy glyph cell 为 8 个 u32；Placement Plan 已将 word
// offset 固定为 cell 起始 word。page-2比例字形是静态资源，动态分支永不允许它。
...
fn vs_main(
  ...
  // Dynamic legacy registers modify only one glyph ID word. The compiler startup proof
  // rejects dynamic page-2 placement, so this legacy cell transform never aliases fontc.
  if (dynamic != 0u) {
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
  // Page selection is compiled into every placement. There is no runtime font lookup,
  // fallback, shaping, atlas coordinate computation, or mutable bind-group selection.
  var coverage: f32;
  if (in.atlas_page == 2) {
